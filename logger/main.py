...
class sx126x:
    ...
    def __init__(self,serial_num,freq,addr,power,rssi):
        ...
        self.ser.flushInput()
        print("Lora Starting...20211123-1556")
    ...
    def lorareceive(self):
        ...
        if self.ser.inWaiting() > 0:
            ...
            messagenode=((r_buff[0]<<8)+r_buff[1])

            print("receive message from address %d node "%messagenode)
            print("message is "+str(r_buff[2:]))
            ...
            lastcode = b[len(b)-1:]
            print(len(b))
            print(int(lastcode[0]))
            if(int(lastcode[0])>210):
             

                try:
                   
                    if (len(self.datasteam)>0):
                        self.datasteam = self.datasteam +  b[0:len(b)].decode("utf-8")
                    else:
                        self.datasteam =b[0:len(b)-1].decode("utf-8")
                    
                    print("receive message from address %d node "%messagenode)   
                    print(self.datasteam)
                    #my_json = inputjson.decode('utf8').replace("'", '"')
                    data_dict = json.loads(self.datasteam)
                    print("JSON : ")
                    print(data_dict)
                    #print the rssi
                ...
                try:
                    if (len(self.datasteam)>0):
                        ...
                        self.datasteam =b[0:len(b)].decode("utf-8")
...
node = sx126x(serial_num = "/dev/ttyS0",freq=433,addr=65535,power=22,rssi=True)
