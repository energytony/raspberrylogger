...
def SettingFileCheck1(fn):
	global setting_data1
	try:

		with open(fn) as json_file:
			setting_data1 = json.load(json_file)
...
def sample_job_every_15s():
	...
	for d in range(len(device_data["device"])):
		...
		if d==0 :
			...
			for i in range(len(setting_data1["field"])):
			 
				try:
					fieldname = setting_data1["field"][i]["name"]
					datatype = setting_data1["field"][i]["type_of_value"]
					
					value = instrument[d].read_long(setting_data1["field"][i]["start_registers_address"], functioncode=3, signed=True, byteorder=0)
					#value = instrument[d].read_float(registeraddress=setting_data1["field"][i]["start_registers_address"],functioncode=3,number_of_registers=2)
...
if __name__ == "__main__":
	...
	#result = SettingFileCheck1("setting-china.json")
	result = SettingFileCheck1("setting-C10.json")
	...
	if (result):
		...
		for i in range(len(device_data["device"])):
			...
			instrument[i].serial.parity   = device_data["device"][i]['setting']['parity']
